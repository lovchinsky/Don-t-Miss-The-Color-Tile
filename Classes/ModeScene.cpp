...
bool ModeScene::init()
{
	...
	auto titleLabel = Label::createWithTTF("Play","Roboto-Light.ttf",MENU_FONT_SIZE);
	...
	keyboardListener->onKeyReleased = CC_CALLBACK_2(ModeScene::onKeyReleased, this);
...
void ModeScene::startGame(Mode mode)
{
...
void ModeScene::classicOnTouchBegan(Touch* touch, Event* event)
{
	startGame(CLASSIC);
}
void ModeScene::arcadeOnTouchBegan(Touch* touch, Event* event)
{
	startGame(ARCADE);
}
void ModeScene::speedOnTouchBegan(Touch* touch, Event* event)
{
	startGame(SPEED);
}
void ModeScene::undefinedOnTouchBegan(Touch* touch, Event* event)
{
	startGame(HARD);
}
void ModeScene::onKeyReleased(EventKeyboard::KeyCode keyCode, cocos2d::Event *event)
{
